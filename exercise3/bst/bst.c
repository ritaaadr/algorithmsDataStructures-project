...
void * bstRemove(BSTObject* tree, DataObject* elem )
{
    ...
    if(!bstExists(tree, elem)){
        ...
        if(elem != NULL && tree->root != NULL) {
          tree->root = tree->type->bst_Remove(tree->root, elem);
            tree->size--;
        }
    }
...
DataObject* bstGetData(BSTObject* tree, DataObject* elemToFind)
{
    assert(tree);

    return tree->type->bst_GetData(tree->root, elemToFind);
}
